...
```{r}
lt.graph.hfd %>%
  filter(OTU %in% LT.Q1.deseq.intersect.surgery$row) %>% 
  summary_by(Abundance ~ OTU + Surgery, FUN = median)
```

```{r}
lt.graph.hfd %>%
  filter(OTU %in% LT.Q1.deseq.intersect.intra$row) %>% 
  summary_by(Abundance ~ OTU + Intranasal_Treatment, FUN = median)
```
...
```{r}
lt.graph.none %>%
  filter(OTU %in% ancom.intersect.nosurgery$taxon) %>% 
  summary_by(Abundance ~ OTU + Intranasal_Treatment, FUN = median)
```
...
lt.graph.none %>%
  filter(OTU == "Pirellula") %>% 
  summary_by(Abundance ~ OTU + Diet, FUN = median)
```
